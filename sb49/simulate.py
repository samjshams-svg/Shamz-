"""Monte Carlo game engine: plays a game out from any GameState."""

from __future__ import annotations

import random
from dataclasses import dataclass

from .model import LeagueModel, punt_zone
from .state import HALF_SECONDS, KICKOFF, OT_SECONDS, PAT, SCRIMMAGE, GameState, game_mode

MAX_SNAPS = 500
KNEEL_SECS = 40


class GameOver(Exception):
    def __init__(self, winner: str):
        self.winner = winner


@dataclass
class SimResult:
    winner: str
    score: dict[str, int]


class Engine:
    """Applies play results to a GameState. Used both by the simulator and to
    apply a user's hand-picked result to a single real play."""

    def __init__(self, model: LeagueModel, rng: random.Random):
        self.m = model
        self.rng = rng

    # ---- bookkeeping ---------------------------------------------------------
    def run_clock(self, s: GameState, secs: float) -> None:
        if s.half <= 2 and s.secs > 120 and s.secs - secs < 120:
            s.secs = 120.0  # two-minute warning stops the clock
        else:
            s.secs = max(0.0, s.secs - secs)

    def score(self, s: GameState, team: str, pts: int, kind: str) -> None:
        s.score[team] += pts
        if s.half > 2:
            if kind == "fg" and s.ot_stage == 0:
                s.ot_stage = 1
            elif kind == "fg" and s.ot_stage == 1 and s.score[s.offense] == s.score[s.defense]:
                s.ot_stage = 2
            else:
                raise GameOver(self.leader(s))

    def leader(self, s: GameState) -> str:
        a, b = s.offense, s.defense
        return a if s.score[a] > s.score[b] else b

    def change_possession(self, s: GameState, new_yardline: int) -> None:
        """Non-scoring change of possession (turnover, punt, downs, missed FG)."""
        if s.half > 2:
            if s.ot_stage == 0:
                s.ot_stage = 2
            elif s.ot_stage == 1:
                raise GameOver(self.leader(s))
        s.swap()
        s.phase = SCRIMMAGE
        s.yardline_100 = min(99, max(1, int(new_yardline)))
        s.down, s.ydstogo = 1, min(10, s.yardline_100)

    def touchdown(self, s: GameState, team: str) -> None:
        if team != s.offense:
            s.swap()
        self.score(s, team, 6, "td")
        s.phase = PAT

    def safety(self, s: GameState) -> None:
        """Offense is tackled in its own end zone."""
        self.score(s, s.defense, 2, "safety")
        s.phase, s.free_kick = KICKOFF, True  # offense free-kicks to the defense

    # ---- play results (also used for user overrides) --------------------------
    def gain(self, s: GameState, yards: int) -> None:
        new_yl = s.yardline_100 - yards
        if new_yl <= 0:
            self.touchdown(s, s.offense)
        elif new_yl >= 100:
            self.safety(s)
        elif yards >= s.ydstogo:
            s.yardline_100, s.down, s.ydstogo = new_yl, 1, min(10, new_yl)
        elif s.down == 4:
            self.change_possession(s, 100 - new_yl)
        else:
            s.yardline_100, s.down, s.ydstogo = new_yl, s.down + 1, s.ydstogo - yards

    def turnover(self, s: GameState, net: int = 0, defense_td: bool = False) -> None:
        """Interception / lost fumble. `net` = how far past the line of
        scrimmage (offense's direction) the defense ends up with the ball."""
        if defense_td:
            self.touchdown(s, s.defense)
            return
        new_yl = 100 - (s.yardline_100 - net)
        if new_yl >= 100:
            new_yl = 80  # touchback
        if new_yl <= 0:
            self.touchdown(s, s.defense)
            return
        self.change_possession(s, new_yl)

    def field_goal(self, s: GameState, made: bool) -> None:
        if made:
            self.score(s, s.offense, 3, "fg")
            s.phase = KICKOFF
        else:
            self.change_possession(s, 100 - max(20, s.yardline_100 + 7))

    def punt(self, s: GameState, net: int, return_td: bool = False) -> None:
        if return_td:
            self.change_possession(s, 50)
            self.touchdown(s, s.offense)
            return
        self.change_possession(s, 100 - (s.yardline_100 - net))

    # ---- decisions -------------------------------------------------------------
    def should_kneel(self, s: GameState) -> bool:
        if s.half != 2 or s.diff <= 0 or s.yardline_100 >= 98:
            return False
        kneels = 5 - s.down
        def_to = s.timeouts[s.defense]
        return s.secs <= KNEEL_SECS * max(0, kneels - def_to) + 2

    def fourth_down(self, s: GameState) -> str:
        yl, togo, diff, secs = s.yardline_100, s.ydstogo, s.diff, s.secs
        fg_range = yl <= 37
        late = s.half == 2 and secs <= 300
        if s.half == 2 and diff < -8 and secs <= 600:
            return "go"
        if late and diff < 0:
            return "fg" if -diff <= 3 and fg_range else "go"
        if s.half > 2 and fg_range:
            return "fg"
        if fg_range:
            return "go" if togo <= 1 and yl <= 2 and diff < 0 else "fg"
        if (yl <= 40 and togo <= 3) or (togo <= 1 and yl <= 55):
            return "go"
        return "punt"

    # ---- simulation loop -------------------------------------------------------
    def play_out(self, s: GameState) -> SimResult:
        try:
            for _ in range(MAX_SNAPS):
                self.step(s)
        except GameOver as end:
            return SimResult(end.winner, dict(s.score))
        teams = list(s.score)
        return SimResult(self.rng.choice(teams), dict(s.score))

    def step(self, s: GameState) -> None:
        if s.phase == PAT:
            self.try_after_td(s)
            return
        if s.secs <= 0:
            self.end_period(s)
            return
        if s.phase == KICKOFF:
            self.kickoff(s)
            return
        self.scrimmage(s)

    def end_period(self, s: GameState) -> None:
        a, b = s.offense, s.defense
        if s.half == 1:
            recv = s.second_half_receiver
            kicker = a if recv == b else b
            s.offense, s.defense = kicker, recv
            s.phase, s.free_kick = KICKOFF, False
            s.half, s.secs = 2, HALF_SECONDS
            s.timeouts = {a: 3, b: 3}
        elif s.half == 2:
            if s.score[a] != s.score[b]:
                raise GameOver(self.leader(s))
            recv = self.rng.choice([a, b])  # coin toss
            s.offense, s.defense = (b, a) if recv == a else (a, b)
            s.phase, s.free_kick = KICKOFF, False
            s.half, s.secs, s.ot_stage = 3, OT_SECONDS, 0
            s.timeouts = {a: 3, b: 3}
        else:
            s.half, s.secs = s.half + 1, OT_SECONDS  # playoffs: keep playing

    def try_after_td(self, s: GameState) -> None:
        diff = s.diff
        go_for_two = s.half == 2 and s.secs <= 900 and diff in (-10, -5, -2, 1, 5, 12)
        if go_for_two:
            if self.rng.random() < self.m.two_pt_rate:
                s.score[s.offense] += 2
        elif self.rng.random() < self.m.pat_rate:
            s.score[s.offense] += 1
        s.phase, s.free_kick = KICKOFF, False

    def kickoff(self, s: GameState) -> None:
        trailing = s.diff < 0
        onside = s.half == 2 and trailing and (s.secs <= 150 or (s.diff < -8 and s.secs <= 300))
        if onside and not s.free_kick:
            self.run_clock(s, 4)
            if self.rng.random() < self.m.onside_recovery:
                s.phase, s.yardline_100, s.down, s.ydstogo = SCRIMMAGE, 55, 1, 10
            else:
                self.change_possession(s, 45)
            return
        yl, td, secs = self.rng.choice(self.m.kickoffs)
        self.run_clock(s, secs)
        s.swap()
        if td:
            self.touchdown(s, s.offense)
            return
        if s.free_kick:
            yl = max(1, yl - 15)
            s.free_kick = False
        s.phase, s.yardline_100 = SCRIMMAGE, int(yl)
        s.down, s.ydstogo = 1, min(10, s.yardline_100)

    def scrimmage(self, s: GameState) -> None:
        off, dfn = s.offense, s.defense
        if self.should_kneel(s):
            if s.timeouts[dfn] > 0:
                s.timeouts[dfn] -= 1
                self.run_clock(s, 2)
            else:
                self.run_clock(s, KNEEL_SECS)
            self.gain(s, -1)
            return

        fg_range = s.yardline_100 <= 37
        mode = game_mode(s.half, s.secs, s.diff)
        end_of_half_kick = (
            fg_range and s.secs <= 8
            and (s.half == 1 or s.half > 2 or -3 <= s.diff <= 0)
        )
        decision = "fg" if end_of_half_kick else (self.fourth_down(s) if s.down == 4 else "go")
        if decision == "fg":
            self.run_clock(s, 5)
            self.field_goal(s, self.rng.random() < self.m.fg_prob(s.yardline_100))
            return
        if decision == "punt":
            pool = self.m.punts[punt_zone(s.yardline_100)]
            net, td, secs = self.rng.choice(pool)
            self.run_clock(s, secs)
            self.punt(s, net, bool(td))
            return

        pool = self.m.scrimmage_pool(s.down, s.ydstogo, s.yardline_100, mode)
        yards, turnover, net, def_td, stops = self.rng.choice(pool)
        durations = self.m.durations.get((mode, stops)) or self.m.durations[("normal", stops)]
        secs = self.rng.choice(durations)

        # Clock management after an in-bounds play.
        if not stops and not turnover:
            if mode == "hurry" and s.timeouts[off] > 0:
                s.timeouts[off] -= 1
                secs = min(secs, 6)
            elif mode == "protect" and s.timeouts[dfn] > 0 and s.secs <= 240:
                s.timeouts[dfn] -= 1
                secs = min(secs, 6)
        self.run_clock(s, secs)

        if turnover:
            self.turnover(s, net, bool(def_td))
        else:
            self.gain(s, int(yards))
